...
export const redisOptions: Record<string, unknown> = {
...
export class RedisService {
	...
	public async getItem(_args: { id: string }): Promise<string | null> {
		return null
	}
	...
	public incrementItem = async (_args: {
		key: string
		incrementValue?: number
		ttl?: number
	}): Promise<number> => {
	...
	public deleteItem = async (_args: { key: string }): Promise<void> => {}
	...
	public getFilePathsFromRedis = async (_args: {
		key: string
		totalChunks: number
	}): Promise<string[]> => {
		return []
	...
	public getTTL = async (_args: { key: string }): Promise<number> => {
		return -1
	}
}
